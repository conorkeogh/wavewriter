...
# %% auto 0
__all__ = ['plot_waveforms', 'convert_waveform', 'generate_tonic', 'generate_burst_boston', 'exponential_decay', 'objective',
           'generate_burst_abbott', 'generate_sine', 'generate_russian', 'generate_wavelet',
...
# %% ../nbs/04_examples.ipynb 49
# Generate sawtooth-modulated waveform
def generate_sawtooth_modulated(
    amplitude: float, # Maximum amplitude (mA)
    frequency_sine: int, # Frequency of sinusoid (Hz)
    frequency_sawtooth: int # Frequency of modulation (Hz)
) -> (np.ndarray, np.ndarray):
    '''
    Generate sawtooth-modulated waveform
    Sinusoidal waveform modulated by sawtooth
    Args:
        amplitude: float; maximum amplitude (mA)
        frequency_sine: int; frequency of sinusoid (Hz)
        frequency_sawtooth: int; frequency of modulation (Hz)
    Returns:
        v: ndarray; output at each timpoint (mA)
        t: ndarray; corresponding timepoints (us)
    '''
    # Set timestep
    timestep = 5 # us
    ...
    # Get total length
    period_sawtooth = 1 / frequency_sawtooth
    num_samples = int((1000000 / frequency_sawtooth) / timestep)
    t = np.linspace(0, period_sawtooth, num=int(num_samples))
    
    # Get sinusoid over total duration
    v = np.sin(2 * np.pi * frequency_sine * t) * amplitude
    
    # Get sawtooth modulation
    sawtooth = np.linspace(0, 1, num=int(num_samples))
    
    # Modulate sinusoid with sawtooth
    v = v * sawtooth
